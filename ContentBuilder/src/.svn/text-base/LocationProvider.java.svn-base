...
public class LocationProvider implements IAppFileLocProvider
{
	...
	public LocationProvider(File aBinPath, File aProfileDir) throws IOException
	{
		libXULPath = aBinPath;
		profile = aProfileDir;
		if (!libXULPath.exists() || !libXULPath.isDirectory()) { throw new FileNotFoundException(
				"libxul directory specified is not valid: "
						+ libXULPath.getAbsolutePath()); }
		if (profile != null && (!profile.exists() || !profile.isDirectory())) { throw new FileNotFoundException(
				"profile directory specified is not valid: "
	...
	protected void setupProfile() throws IOException
	{
	...
	{
		File file = null;
		if (aProp.equals("Grep") || aProp.equals("GreComsD"))
		{
			file = libXULPath;
